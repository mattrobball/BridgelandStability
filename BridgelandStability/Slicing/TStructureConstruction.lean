    ...
    · push Not at hall_pos
      ...
      · -- Mixed case: some phases > 0 and some ≤ 0
        ...
        have hchainN : F.chain.obj' (F.n - 1 + 1) (by lia) =
          ...
          congrArg F.chain.obj (Fin.ext (by simp [Fin.val_last]; lia))
        let e₂A : T.obj₂ ≅ A :=
        ...
        have hφlast : F.φ ⟨F.n - 1, by lia⟩ ≤ 0 :=
          le_trans (F.hφ.antitone (Fin.mk_le_mk.mpr (by lia))) hj₀
        ...
        · -- Y' is zero (prefix was all-positive): f' is iso
          ...
          let eX : X ≅ F.chain.obj' (F.n - 1) (by lia) := asIso f'
          have hGn0 : 0 < G.n := by change 0 < F.n - 1; lia
          ...
          · -- Phase containment: single.φ j = F.φ(n-1)
            intro j; exact ⟨⟨F.n - 1, by lia⟩, by simp [HNFiltration.single]⟩
        · -- Y' has filtration with phase bound: use octahedral + appendFactor
          -- Extract the phase bound from the IH
          have hGn : 0 < G.n := by change 0 < F.n - 1; lia
          have hφlast_lt : ∀ j : Fin GY'.n,
              F.φ ⟨F.n - 1, by lia⟩ < GY'.φ j := by
            intro j
            calc F.φ ⟨F.n - 1, by lia⟩
                < F.φ ⟨F.n - 2, by lia⟩ :=
          ...
          refine ⟨X, Z, hX, f' ≫ u₂₃, v₁₃, w₁₃, h₁₃,
            hXdata.imp id (fun ⟨GX, hGX, hpos, hbd, hcontain⟩ ↦
              ⟨GX, hGX, hpos, fun _ ↦ hbd (by change 0 < F.n - 1; lia), fun j ↦ by
                obtain ⟨i, hi⟩ := hcontain j
                have hi_lt := i.isLt; change i.val < F.n - 1 at hi_lt
                exact ⟨⟨i.val, by lia⟩, by simp [G, HNFiltration.prefix] at hi; exact hi⟩⟩),
    ...
    · push Not at hall_nonneg
      ...
      · push Not at hall_neg
        ...
        have hTu₂₃ : Triangle.mk u₂₃ (e₂A.inv ≫ T.mor₂)
          (T.mor₃ ≫ e₁.hom⟦(1 : ℤ)⟧') ∈ distTriang C :=
          isomorphic_distinguished _ (F.triangle_dist ⟨F.n - 1, by lia⟩) _ Tisoₘ
        have hφlast : F.φ ⟨F.n - 1, by lia⟩ < 0 := by
          obtain ⟨j, hj⟩ := hall_nonneg
          exact lt_of_le_of_lt
            (F.hφ.antitone (Fin.mk_le_mk.mpr (by lia))) hj
        obtain ⟨Z, v₁₃, w₁₃, h₁₃⟩ := distinguished_cocone_triangle (f' ≫ u₂₃)
        let oct := Triangulated.someOctahedron rfl hT' hTu₂₃ h₁₃
        have hLast : s.ltProp C 0 T.obj₃ := by
          exact s.ltProp_of_hn C
            (HNFiltration.single C T.obj₃ (F.φ ⟨F.n - 1, by lia⟩)
              (F.semistable ⟨F.n - 1, by lia⟩))
            0 (fun _ ↦ hφlast) (by
              change 0 < 1
              lia)
