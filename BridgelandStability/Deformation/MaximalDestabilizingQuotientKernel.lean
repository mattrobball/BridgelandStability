...
theorem SkewedStabilityFunction.semistable_cokernel_of_minPhase_strictKernel_of_minimal
  ...
  have hpb_phase_lt :
      wPhaseOf (ssf.W (cl C v (cokernel pbB.arrow).obj)) ssf.α < ψY := by
    rw [ssf.Wobj_cokernel_pullback_eq
      (C := C) (s := σ.slicing) (a := a) (b := b) (X := X) M hM_strict
      (B := B) hB_strict]
    exact hcokB_phase_lt
  have hpb_phase_ge :
      ψY ≤ wPhaseOf (ssf.W (cl C v (cokernel pbB.arrow).obj)) ssf.α :=
...
theorem SkewedStabilityFunction.phase_lt_of_strictQuotient_of_minPhase_strictKernel
  ...
  let ψM : ℝ := wPhaseOf (ssf.W (cl C v (cokernel M.arrow).obj)) ssf.α
  let ψLift : ℝ := wPhaseOf (ssf.W (cl C v (cokernel liftA.arrow).obj)) ssf.α
  ...
  have hA_range :
    ...
    constructor <;> linarith
  have hM_Wne : ssf.W (cl C v (cokernel M.arrow).obj) ≠ 0 :=
  ...
  have hA_phase_gt_lift :
      ψLift < wPhaseOf (ssf.W (cl C v (cokernel A.arrow).obj)) ssf.α :=
  ...
  | succ k ih =>
      ...
      · obtain ⟨M, hM_top, hM_strict, hM_min, hM_lt⟩ :=
        ...
        · subst hM_bot
          have hss_bot :
              ssf.Semistable C (cokernel ((⊥ : Subobject Y).arrow)).obj
                (wPhaseOf (ssf.W (cl C v (cokernel ((⊥ : Subobject Y).arrow)).obj)) ssf.α) :=
        ...
        · have hM_ne : ¬IsZero (M : σ.slicing.IntervalCat C a b) :=
          ...
          let ψQ : ℝ := wPhaseOf (ssf.W (cl C v (cokernel M.arrow).obj)) ssf.α
          have hψQ_gt : t < ψQ := by
          ...
          have hquot_M :
              ∀ A : Subobject (M : σ.slicing.IntervalCat C a b), A ≠ ⊤ → IsStrictMono A.arrow →
                ψQ < wPhaseOf (ssf.W (cl C v (cokernel A.arrow).obj)) ssf.α := by
            intro A hA_top hA_strict
            simpa [ψQ] using
...
theorem wPhaseOf_cokernel_mapMono_eq
    ...
    wPhaseOf
      ...
      wPhaseOf
        (ssf.W (cl C v (cokernel (Subobject.ofLE S T h)).obj)) ssf.α := by
  let eC :=
...
theorem wPhaseOf_cokernel_ofLE_top_eq
    ...
    [HasCokernels (s.IntervalCat C a b)]
    {X : s.IntervalCat C a b} (A : Subobject X) :
    wPhaseOf (W (cl C v (cokernel (Subobject.ofLE A ⊤ le_top)).obj)) α =
...
theorem wPhaseOf_cokernel_kernelSubobject_eq
    ...
    wPhaseOf (ssf.W (cl C v (cokernel (kernelSubobject q).arrow).obj)) ssf.α =
      wPhaseOf (ssf.W (cl C v B.obj)) ssf.α := by
  ...
  have hsumB :
      ssf.W (cl C v E.obj) =
        ssf.W (cl C v (kernelSubobject q : s.IntervalCat C a b).obj) +
          ssf.W (cl C v B.obj) := by
    simpa [map_add] using
      ssf.strict_additive
        (C := C) (s := s) (a := a) (b := b)
  ...
  have hsumC :
      ssf.W (cl C v E.obj) =
        ssf.W (cl C v (kernelSubobject q : s.IntervalCat C a b).obj) +
          ssf.W (cl C v (cokernel (kernelSubobject q).arrow).obj) := by
    simpa [map_add] using
      ssf.strict_additive
        (C := C) (s := s) (a := a) (b := b)
        (interval_strictShortExact_cokernel_of_strictMono
  ...
  have hWB :
    ...
    apply add_left_cancel
      (a := ssf.W (cl C v (kernelSubobject q : s.IntervalCat C a b).obj))
