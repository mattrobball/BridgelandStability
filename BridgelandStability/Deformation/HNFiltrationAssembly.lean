...
theorem semistable_of_hn_length_one
  ...
  have hobj₂_eq : GY.chain.obj' (0 + 1) (by lia) = GY.chain.obj (Fin.last GY.n) :=
...
theorem append_hn_filtration_of_triangle
  ...
  | succ m ih =>
      ...
      · have hYpos : 0 < GY.n := Nat.pos_of_ne_zero hYn
        ...
        · have hYtwo : 2 ≤ GY.n := by lia
          let jLast : Fin GY.n := ⟨GY.n - 1, by lia⟩
          let GY' := GY.prefix C (GY.n - 1) (by lia) (by lia)
          let Tlast := GY.triangle jLast
          let e₁ := Classical.choice (GY.triangle_obj₁ jLast)
          let e₂ := Classical.choice (GY.triangle_obj₂ jLast)
          let eY := by
            have hchainN : GY.chain.obj' (GY.n - 1 + 1) (by lia) =
                GY.chain.obj (Fin.last GY.n) :=
              congrArg GY.chain.obj (Fin.ext (by simp [Fin.last]; lia))
            exact e₂.trans ((eqToIso hchainN).trans (Classical.choice GY.top_iso))
          let f23 : GY.chain.obj ⟨GY.n - 1, by lia⟩ ⟶ Y :=
            e₁.inv ≫ Tlast.mor₁ ≫ eY.hom
          let g23 : Y ⟶ Tlast.obj₃ :=
            eY.inv ≫ Tlast.mor₂
          let h23 : Tlast.obj₃ ⟶ GY.chain.obj ⟨GY.n - 1, by lia⟩⟦(1 : ℤ)⟧ :=
            Tlast.mor₃ ≫ e₁.hom⟦(1 : ℤ)⟧'
          ...
          have hY'_gt_last : ∀ i : Fin GY'.n, GY.φ jLast < GY'.φ i := by
            ...
            have hi : i.val < GY.n - 1 := by
              simpa [hEqn] using i.is_lt
            change GY.φ jLast < GY.φ ⟨i.val, by lia⟩
            exact GY.hφ (show (⟨i.val, by lia⟩ : Fin GY.n) < jLast by
      ...
      · have hYpos : 0 < GY.n := Nat.pos_of_ne_zero hYn
        ...
        · have hYtwo : 2 ≤ GY.n := by lia
          ...
          have hY'_gt_last : ∀ i : Fin GY'.n, GY.φ jLast < GY'.φ i := by
            ...
            have hi : i.val < GY.n - 1 := i.is_lt
            change GY.φ jLast < GY.φ ⟨i.val, by lia⟩
            exact GY.hφ (Fin.mk_lt_mk.mpr (by lia))
...
theorem split_hn_filtration_at_cutoff
  ...
  | zero =>
      ...
      · intro hn0 j; exact False.elim (by lia)
  ...
  | succ m ih =>
      ...
      · have hn0 : 0 < F.n := Nat.pos_of_ne_zero hn
        by_cases hlast_gt : t < F.φ ⟨F.n - 1, by lia⟩
        · refine ⟨A, 0, F, HNFiltration.zero C (P := P) 0 (isZero_zero C),
            𝟙 A, 0, 0, contractible_distinguished A, ?_, ?_, ?_, ?_⟩
          ...
          · intro j; exact ⟨j, rfl⟩
        · have hlast_le : F.φ ⟨F.n - 1, by lia⟩ ≤ t := by
            linarith
          ...
          · have hn2 : 2 ≤ F.n := by lia
            ...
            have hGn : 0 < G.n := by
              change 0 < F.n - 1
              lia
            have hφlast_lt : ∀ j : Fin GY'.n, F.φ ⟨F.n - 1, by lia⟩ < GY'.φ j := by
              intro j
              calc
                F.φ ⟨F.n - 1, by lia⟩
                    < F.φ ⟨F.n - 2, by lia⟩ :=
                  F.hφ (show (⟨F.n - 2, by lia⟩ : Fin F.n) <
                    ⟨F.n - 1, by lia⟩ from
                      Fin.mk_lt_mk.mpr (by lia))
                _ = G.φ ⟨G.n - 1, by lia⟩ := by
            ...
            let GZ := GY'.appendFactor C oct.triangle oct.mem (Iso.refl _)
              (Iso.refl _) (F.φ ⟨F.n - 1, by lia⟩)
              (F.semistable ⟨F.n - 1, by lia⟩) hφlast_lt
            ...
            · intro j
              ...
              · exact hlast_le
            · intro _ j
              change F.φ ⟨F.n - 1, by lia⟩ ≤ GZ.φ j
              simp only [GZ, HNFiltration.appendFactor]
              split_ifs with hj
