...
**Note.** `closedUnderIso` and `zero_mem` are not explicit in Bridgeland's
definition but follow from P(φ) being a full additive subcategory. The
formalization uses `ObjectProperty` (a predicate on objects) rather than a
subcategory, so these are stated explicitly. `hn_exists` quantifies over ALL
objects (including zero), returning `Nonempty` rather than requiring a
...
**Note.** Well-definedness (`phiPlus_eq`, `phiMinus_eq`) is proved in
`Slicing/Phase.lean` but is NOT in the challenge path. The definitions use
`Classical.choice` to pick a witness.
...
**Note.** "Finite length" is formalized as strict-Artinian ∧ strict-Noetherian.
In a quasi-abelian category, strict subobjects (via strict monos) may be a proper
subset of all subobjects (via monos). The formalization adds the bound `η < 1/2`
to ensure the interval width stays ≤ 1, which is needed for the quasi-abelian
structure on P((a,b)).

**Audit question.** Does Bridgeland mean finite length w.r.t. strict or all
subobjects? In an abelian category they coincide. For quasi-abelian
categories, strict is standard but weaker.
...
**Note.** The class-map version factors Z through v : K₀(C) → Λ. The ordinary
...
**Audit question.** The paper does not restrict ε to (0, 1/8). The bound
ensures certain sector estimates hold. Does `generateFrom` with `ε < 1/8`
produce the same topology as using all ε > 0? It should, since B_ε(σ) nests
downward, but this depends on the neighborhoods actually forming a basis (not
just a sub-basis) — which is Proposition 6.2.
...
**Audit question.** The induced topology pulls back along `toStabilityCondition`,
which composes Z with v. If v is not injective, two stability conditions with
different Z : Λ →+ ℂ but same Z ∘ v could be topologically indistinguishable.
For the numerical case, v = numericalQuotientMap is surjective, so
toStabilityCondition is injective and the induced topology is Hausdorff. For
non-surjective v, this topology may be too coarse.
...
**Note.** Triangle-additivity in both arguments is proved in
...
**Note.** `eulerFormRad` is the LEFT radical (kernel of E ↦ χ(E, −)). The Euler
form is not symmetric in general (χ(E, F) ≠ χ(F, E)), so left radical ≠ right
radical. Bridgeland uses the left radical.
...
1. **IsLocallyFinite: strict vs ordinary finite length.** The formalization uses
   `IsStrictArtinianObject ∧ IsStrictNoetherianObject`. In a quasi-abelian
   category, this is standard for "finite length" but strictly weaker than
   Artinian ∧ Noetherian. Does Bridgeland intend strict or ordinary?

2. **Topology ε-bound.** `generateFrom` with `ε < 1/8` should produce the same
   topology as all ε > 0, but this relies on the neighborhoods forming a basis
   (Proposition 6.2), which is outside the challenge path.

3. **Induced topology for non-surjective v.** The topology on `WithClassMap C v`
   is too coarse when v is not surjective. For Corollary 1.3, v is surjective
   so this is not an issue.

4. **Existential model space.** The manifold theorem existentially quantifies E
   and its normed space structure, hiding the dimension. For finite-dimensional
   E over ℂ, the topology is unique (all norms equivalent). The dimension bound
   dim ≤ rank N(D) from the paper is not asserted.

5. **Vacuity.** The theorem is vacuously true when no numerical stability
   conditions exist (empty ComponentIndex). This matches the paper, which also
   does not prove existence. Existence is a separate hard problem
   (proved for curves in §9, K3 surfaces in later work).
