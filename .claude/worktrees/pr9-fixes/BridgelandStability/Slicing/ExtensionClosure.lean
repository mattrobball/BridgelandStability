...
lemma Slicing.gtProp_of_triangle (s : Slicing C) {A E B : C} (t : ℝ)
  ...
  -- All maps A → F⁻ vanish
  have hA_vanish :
      ∀ α : A ⟶ (FE.triangle ⟨FE.n - 1, by lia⟩).obj₃, α = 0 := by
    intro α
    rcases hA with hAZ | ⟨FA, hnA, hFA_gt⟩
    · exact hAZ.eq_of_src α 0
    · exact s.hom_eq_zero_of_lt_phases C
        (FE.semistable ⟨FE.n - 1, by lia⟩) FA
        (fun i ↦ lt_of_le_of_lt hle
          (lt_of_lt_of_le hFA_gt
            (FA.hφ.antitone (Fin.mk_le_mk.mpr (by lia))))) α
  -- All maps B → F⁻ vanish
  have hB_vanish :
      ∀ β : B ⟶ (FE.triangle ⟨FE.n - 1, by lia⟩).obj₃, β = 0 := by
    intro β
    rcases hB with hBZ | ⟨FB, hnB, hFB_gt⟩
    · exact hBZ.eq_of_src β 0
    · exact s.hom_eq_zero_of_lt_phases C
        (FE.semistable ⟨FE.n - 1, by lia⟩) FB
        (fun i ↦ lt_of_le_of_lt hle
          (lt_of_lt_of_le hFB_gt
            (FB.hφ.antitone (Fin.mk_le_mk.mpr (by lia))))) β
  -- All maps E → F⁻ vanish (by Yoneda exactness)
  have hE_vanish :
      ∀ γ : E ⟶ (FE.triangle ⟨FE.n - 1, by lia⟩).obj₃, γ = 0 := by
    intro γ
    obtain ⟨δ, hδ⟩ :=
...
lemma Slicing.phiMinus_gt_of_triangle (s : Slicing C) {A E B : C}
  ...
  have hA_vanish :
    ...
    · obtain ⟨FA, hnA, hneA⟩ :=
      ...
      have hFA_gt : t < FA.φ ⟨FA.n - 1, by lia⟩ := by
  ...
  have hB_vanish :
    ...
    · obtain ⟨FB, hnB, hneB⟩ :=
      ...
      have hFB_gt : t < FB.φ ⟨FB.n - 1, by lia⟩ := by
...
lemma Slicing.intervalProp_of_triangle (s : Slicing C) {A E B : C} {a b : ℝ}
  ...
  exact ⟨F, fun i ↦ ⟨by
    calc a < s.phiMinus C E hEZ := hMinus
      _ = F.φ ⟨F.n - 1, by lia⟩ := s.phiMinus_eq C E hEZ F hn hlast
...
theorem Slicing.phiMinus_triangle_le (s : Slicing C) {A E B : C}
  ...
  have hB_factor_zero :
      ∀ α : B ⟶ (FB.triangle ⟨FB.n - 1, by lia⟩).obj₃, α = 0 := by
    intro α
    -- g ≫ α : E → B⁻ is zero by hom-vanishing
    have hgα : g ≫ α = 0 :=
      s.hom_eq_zero_of_lt_phases C (FB.semistable ⟨FB.n - 1, by lia⟩) FE hE_gap _
    ...
    · -- Get an HN filtration of A⟦1⟧ from hA_int
      ...
      · -- Shift GA by 1 to get filtration of A⟦1⟧
        ...
        have hAs_gap : ∀ j : Fin GAs.n,
            FB.φ ⟨FB.n - 1, by lia⟩ < GAs.φ j := by
          intro j
          change FB.φ ⟨FB.n - 1, by lia⟩ < GA.φ j + ((1 : ℤ) : ℝ)
          have h1 : GA.φ j > a := (hGA j).1
          have h2 : FB.φ ⟨FB.n - 1, by lia⟩ < b := by
            calc FB.φ ⟨FB.n - 1, by lia⟩
                = s.phiMinus C B hB :=
