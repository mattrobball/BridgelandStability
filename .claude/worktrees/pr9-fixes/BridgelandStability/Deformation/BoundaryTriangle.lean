  ...
  · have hY_minus : a < s.phiMinus C Y hYZ :=
    ...
    exact Or.inr ⟨F, fun i ↦ ⟨by
      calc
        ...
        _ = F.φ ⟨F.n - 1, by lia⟩ := s.phiMinus_eq C Y hYZ F hn hlast
        _ ≤ F.φ i := F.hφ.antitone (Fin.mk_le_mk.mpr (by lia)), by
        calc
          F.φ i ≤ F.φ ⟨0, hn⟩ :=
...
theorem wPhaseOf_gt_of_geProp_target
  ...
  have hphases : ∀ i : Fin F.n, ψ + ε₀ ≤ F.φ i ∧ F.φ i < b := by
    ...
    · calc
        ...
        _ = F.φ ⟨F.n - 1, by lia⟩ := σ.slicing.phiMinus_eq C E hEne F hn hlast
...
theorem wPhaseOf_lt_of_lower_boundary_triangle
  ...
  have hY_le' : σ.slicing.leProp C (ψ - ε₀) Y :=
    ((σ.slicing.leProp_mono (C := C) (t₁ := a₁) (t₂ := ψ - ε₀) (by linarith)) Y) hY_le
